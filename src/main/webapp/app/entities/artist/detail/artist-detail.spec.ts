...
describe('Artist Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./artist-detail').then(m => m.ArtistDetail),
              resolve: { artist: () => of({ id: 5568 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load artist on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', ArtistDetail);

      // THEN
      expect(instance.artist()).toEqual(expect.objectContaining({ id: 5568 }));
    });
  });
