...
describe('Genre Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 2628 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.genres()[0]).toEqual(expect.objectContaining({ id: 2628 }));
  });

  describe('trackId', () => {
    it('should forward to genreService', () => {
      const entity = { id: 2628 };
      vitest.spyOn(service, 'getGenreIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.genresParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
