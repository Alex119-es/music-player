...
import { sampleWithRequiredData } from '../like.test-samples';
...
describe('Like Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 24446 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.likes()[0]).toEqual(expect.objectContaining({ id: 24446 }));
  });

  describe('trackId', () => {
    it('should forward to likeService', () => {
      const entity = { id: 24446 };
      vitest.spyOn(service, 'getLikeIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.likesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
