...
import { sampleWithRequiredData } from '../play.test-samples';
...
describe('Play Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 15119 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.plays()[0]).toEqual(expect.objectContaining({ id: 15119 }));
  });

  describe('trackId', () => {
    it('should forward to playService', () => {
      const entity = { id: 15119 };
      vitest.spyOn(service, 'getPlayIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPlayIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.playsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
