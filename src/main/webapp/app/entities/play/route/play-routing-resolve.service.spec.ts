...
import playResolve from './play-routing-resolve.service';
...
describe('Play routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPlay returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          playResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(playResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
