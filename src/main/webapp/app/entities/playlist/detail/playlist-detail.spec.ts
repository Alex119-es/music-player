...
describe('Playlist Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./playlist-detail').then(m => m.PlaylistDetail),
              resolve: { playlist: () => of({ id: 26585 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load playlist on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', PlaylistDetail);

      // THEN
      expect(instance.playlist()).toEqual(expect.objectContaining({ id: 26585 }));
    });
  });
