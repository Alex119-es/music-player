...
import { sampleWithRequiredData } from '../playlist.test-samples';
...
describe('Playlist Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26585 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.playlists()[0]).toEqual(expect.objectContaining({ id: 26585 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 26585 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.playlists()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 26585 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to playlistService', () => {
      ...
      vitest.spyOn(service, 'getPlaylistIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.playlistsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
