...
describe('Playlist routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPlaylist returned by find', async () => {
      // GIVEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(playlistResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
