...
import { sampleWithRequiredData } from '../song.test-samples';
...
describe('Song Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 123 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 123 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.songs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 123 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to songService', () => {
      const entity = { id: 123 };
      vitest.spyOn(service, 'getSongIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.songsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
